...
import DocumentosList from '../lists/DocumentosList';
import DocumentosNewModal from '../DocumentosNewModal';
import DocumentosEditModal from '../DocumentosEditModal';
...
const DocumentosView = () => {
  ...
  const [newItem, setNewItem] = useState({ titulo: '', descripcion: '', contenido: '', tipo: 'otro', archivoFile: null });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setData(resData.documentos || (Array.isArray(resData) ? resData : []));
    } catch (err) {
  ...
  const filteredData = useMemo(() => {
    ...
    return filtered.filter(item => {
      ...
      const fallback = normalize(`${item.titulo||''} ${item.descripcion||''} ${item.tipo||''}`);
